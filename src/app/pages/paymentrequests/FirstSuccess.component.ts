...
export class FirstSuccessComponent  {
  ...
  async pdfChallan(applicationId){
    ...
    // var generatereceipt = await this.api.OnlinePaymentChallan('1',this.transaction_id,this.payment_amount,this.payment_status,this.application_id,this.payment_date_time,this.enrollment_no,this.user_id);
    // generatereceipt.subscribe(
    //   data => {
    //     var value = data['data'].split('/').pop();
    //     this.api.downloadFiles(value)
